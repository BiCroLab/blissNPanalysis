---
title: "Tutorial on BLISS Downstream Analysis (human)"
...
png(filename=file.path("images", "fig1d_human.png"), units="in", width=8, height=8, res=300)
    ...
    for( treatment in sampleTable[, rev(unique(Treatment))] ){
        ...
        for( name in sampleTable[Treatment==treatment, name] ){
            ...
            circos.genomicLines(region=pl[[name]][chr==chrom,], value=pl[[name]][chr==chrom,],
...
pl = rbindlist(pl, idcol="sample")
pl[, c("Exp", "Tre", "Rep") := tstrsplit(sample, "_")]
pl = dcast(pl, Exp+chr+start+end+Rep~Tre, value.var="value")
pl[, log2FC := log2((ETO+1)/(DMSO+1))]

pl = pl[, .(chr, start, end, Exp, value=log2FC)]

png(filename=file.path("images", "fig1d_bis_human.png"), units="in", width=8, height=8, res=300)
    sel_col = get_palette("Paired", nrow(sampleTable))
    i = 1
    circos.initializeWithIdeogram(species = genome, chromosome.index = paste0("chr", chromosomes))
    for( exp in pl[, unique(Exp)] ){
        circos.track(factors=paste0("chr", chromosomes), ylim=c(-2.5, 2), track.height = 0.1)
            for( chrom in pl[Exp==exp, unique(chr)] )
            circos.genomicLines(region=pl[Exp==exp & chr==chrom,], value=pl[Exp==exp & chr==chrom,],
                                numeric.column=5, col = sel_col[i], track.index=i+2,
                                sector.index=chrom)
        i = i+1
    }
    text(0, 0, "TK6\nDSBs Genomic Profile", cex = 0.8)
