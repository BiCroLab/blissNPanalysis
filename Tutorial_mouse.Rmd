...
ggsave(fig1a, filename=file.path("images", "fig1a_mouse.pdf"), units="in", width=8, height=6, dpi=300, device=cairo_pdf)
```
...
ggsave(fig1b, filename=file.path("images", "fig1b_mouse.pdf"), units="in", width=8, height=8, dpi=300, device=cairo_pdf)
```
...
ggsave(fig1c, filename=file.path("images", "fig1c_mouse.pdf"), units="in", width=10, height=6, dpi=300, device=cairo_pdf)
```
...
cairo_pdf(filename=file.path("images", "fig1d_mouse.pdf"), width=8, height=8)
...
Load expression data and select top and bottom 5% of protein-coding genes:
```{r}
...
Calculate library sizes, which will be used to normalise the signal, and generate DSB metadata profiles across the body of most and least expressed genes:
```{r}
...
ggsave(fig1e, filename=file.path("images", "fig1e_mouse.pdf"), units="in", width=10, height=8, dpi=300, device=cairo_pdf)
```

![](images/fig1e_mouse.png)

Generate DSB metadata profiles around the transcriptional start site (TSS) of most and least expressed genes:
```{r}
num_bins = 100
...
ggsave(fig1f, filename=file.path("images", "fig1f_mouse.pdf"), units="in", width=10, height=8, dpi=300, device=cairo_pdf)
```
